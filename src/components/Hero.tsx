  ...
  useEffect(() => {
    ...
    const initParticleSystem = async () => {
      ...
      const noise4D = (x: number, y: number, z: number, w: number) => 
        Math.sin(x * 0.1 + w) * Math.cos(y * 0.1 + w) * Math.sin(z * 0.1 + w);
      ...
      function generatePyramid(count: number, size: number) {
        ...
        for (let i = 0; i < count; i++) {
          if (Math.random() < 0.5) {
            ...
            points[i * 3] = x * (1 - (y + height/2) / height);
            points[i * 3 + 1] = y;
            points[i * 3 + 2] = z * (1 - (y + height/2) / height);
          }
        }
        return points;
      }
        ...
        particlesMaterial = new THREE.ShaderMaterial({
          ...
          fragmentShader: `
            ...
            void main() {
              ...
              vec3 finalColor = vColor * (1.0 + vEffectStrength * 0.6);
      ...
      // Click to morph
      canvasRef.current.addEventListener('click', triggerMorph);
